...
def actualizar_estado_pedido(pedido, productos, nuevo_estado):
    ...
    supabase.table("pedidos").update({"estado": nuevo_estado, "productos": actualizados}).eq("id", pedido.get("id")).execute()


def cargar_notificaciones(user):
    if not user:
        return []
    try:
        return supabase.table("notificaciones").select("*").eq("user_id", user.id).eq("leida", False).order("created_at", desc=True).execute().data or []
    except Exception as ex:
...
def marcar_notificacion_leida(noti_id):
    supabase.table("notificaciones").update({"leida": True}).eq("id", noti_id).execute()
